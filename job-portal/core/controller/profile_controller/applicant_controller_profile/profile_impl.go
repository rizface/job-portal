package applicant_controller_profile
...
func (p *profile) GetDetail(w http.ResponseWriter, r *http.Request) {
	userId := helper.GetTokenValue(r.Context().Value("applicant-data"),"id")
...
func (p *profile) UpdateDetail(w http.ResponseWriter, r *http.Request) {
	applicant := request.NewApplicant()
	userId := helper.GetTokenValue(r.Context().Value("applicant-data"),"id")
	...
	result := p.service.UpdateDetail(userId, applicant)
